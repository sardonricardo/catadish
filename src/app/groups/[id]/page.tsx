...
export default function GroupDetailPage() {
  ...
  const handleUpdateGroup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!updatedRows || updatedRows.length === 0) {
      setError('No tienes permisos para editar este grupo (RLS).')
      setSavingGroup(false)
      return
    }
  ...
  const handleDeleteGroup = async () => {
    ...
    if (!deletedRows || deletedRows.length === 0) {
      setError('No tienes permisos para borrar este grupo (RLS).')
      setDeletingGroup(false)
      return
    }
  ...
  const handleCopyMessage = async (invite: GroupInviteItem) => {
    const link = getInviteLink(invite.token)
    const text = `Te invito a mi grupo en Catadish. Únete desde aquí: ${link}`
  ...
  if (loading) {
    return <p className="text-sm text-slate-600">Cargando grupo...</p>
  }
  ...
  return (
    <section className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-slate-100 p-6 shadow-sm">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          {editingGroup ? (
            ...
            <>
              <h1 className="text-3xl font-bold tracking-tight text-slate-900">{group?.name ?? 'Grupo'}</h1>
      ...
      <form onSubmit={handleInvite} className="rounded-xl border border-slate-200 bg-white p-5 space-y-3">
        <h2 className="text-lg font-semibold text-slate-900">Crear invitación por link</h2>
        <p className="text-sm text-slate-600">
          Genera un link compartible al instante. Cualquiera con una cuenta podrá unirse con ese enlace.
        </p>
        ...
        {lastInviteLink && (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm">
            <p className="font-semibold text-emerald-800">Link de invitación creado</p>
        ...
        <div className="space-y-2">
          {invites.map((invite) => (
            <div key={invite.id} className="rounded-lg border border-slate-200 bg-white p-3 text-sm">
              <p className="font-medium text-slate-900">{invite.email ?? 'Invitación abierta por link'}</p>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  <a
                    href={`https://wa.me/?text=${encodeURIComponent(
                      `Te comparto un link para unirte a mi grupo en Catadish: ${getInviteLink(invite.token)}`,
                    )}`}
                    target="_blank"
                    rel="noreferrer"
      ...
      <ConfirmModal
        ...
        title="Borrar grupo"
        description="Eliminarás el grupo, sus miembros e invitaciones. Esta acción no se puede deshacer."
        confirmLabel="Sí, borrar grupo"
